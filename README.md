...
4. 利用した図書館、その日の状態、今日やったこと（任意）、メモ（任意）を入力します。事前のTODOや予定との比較は不要で、作業内容が空欄でも保存できます。
...
新規日報（過去の記録の追加を含む）は `actualTaskText` を「今日やったこと」として
任意入力します。未入力時は空文字です。予定なしで取り組む利用を正常ケースとし、
予定作成の質問・予定テキスト入力・予定との比較選択は表示しません。

既存の必須キーは削除せず、新規レコードには `plannedTaskCreated: false`、
`plannedTaskText: ""`、`completionStatus: "not_planned"`（予定なし）を保存します。
従来の3種類の `completionStatus` も型・mapper・Rulesで引き続き受け入れます。
過去の予定・比較結果は詳細と編集画面の「以前の予定・比較記録」から参照でき、
通常編集・翌日追記では文字列の空白も含めて保持します。既存データの一括変換は不要です。

リリース時は **Firestore Rulesを先に反映してから、新しいアプリを公開**してください。
新Rulesは旧アプリの3種類の値も受け入れますが、旧Rulesでは `not_planned` の保存が
拒否されます。旧アプリのmapperは新しい値を読み込めないため、公開後は開いたままの
旧画面も再読み込みして利用します。Rulesだけを旧版へ戻すと新形式の更新が拒否されます。
